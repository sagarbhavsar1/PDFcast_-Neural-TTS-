 ...
 * @returns {Promise<boolean|object>} - Success/failure or error object
 */
async function synthesizeSpeech(text, voice, outputFile, engine = "neural") {
  try {
    ...
    if (error.code === "AccessDeniedException") {
      console.error(`Budget limit exceeded for ${engine} engine. AWS Polly access has been restricted.`);
      return {
        success: false,
        error: "budget_exceeded",
        message: `Monthly budget for neural TTS has been reached. Service will resume next month.`
      };
    }
